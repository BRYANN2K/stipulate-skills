...
The command refuses stale evidence, changed HEAD, concurrent target-spec edits, staged unrelated work or selected files that already had modifications at apply start. Do not reset, overwrite or absorb other work to satisfy it. Reconcile the Git situation with the user when it cannot be safely separated. If HEAD moved, core v1 requires explicit reconciliation/restarting the change; it does not automatically rebase evidence.
