...
class InstallTests(unittest.TestCase):
    ...
    def test_install_run_update_uninstall(self):
        ...
        script=self.dest/'stip-bootstrap/scripts/workflow.py'
        ...
        self.assertEqual(list(self.dest.glob('stip-*')),[])
    def test_foreign_directory_preserved(self):
        p=self.dest/'stip-apply';p.mkdir(parents=True);(p/'custom').write_text('keep')
        self.install(ok=False);self.assertEqual((p/'custom').read_text(),'keep')
    def test_modified_installation_preserved(self):
        self.install();p=self.dest/'stip-apply/SKILL.md';p.write_text('user changes')
