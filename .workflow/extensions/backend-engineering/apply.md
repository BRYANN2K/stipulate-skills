...
Trace inputs, validation, authorization, transactions, and external effects. Define invariants, errors, and transaction boundaries. Implement required concurrency, idempotency, cancellation, or recovery with explicit dependencies. Exercise the real path and failures, verify observability without secrets, then document recovery and limitations. Avoid architectural rewrites not required by the contract.
